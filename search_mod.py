...
#         helper = list(self.path1)
#         helper.append(action)
...
def compare_searchers(h, problems, header,
                      searchers=[
                                 astar_search] ):
    def do(searcher, problem, h):
    ...
    table = [[name(s)] + [do(s, problems,h)] for s in searchers]
    print_table(table, header)
    
    
    
    
def compare2(problem, searchers, heuristics=None):    
#     p = InstrumentedProblem(problem)
    table =[]
    if heuristics == None:
        for s in searchers:            
        ...
        for s in searchers:
            for h in heuristics:
                p = InstrumentedProblem(problem)
                s(p, h)
                table +=[[name(s)]+[name(h)]+ [p]]
    
#     table = [[name(s)] + [do(s, p) for p in problem] for s in searchers]
    print_table(table, ['Searcher','heuristic','Results(succs, goal_tests, states, last)'] )
